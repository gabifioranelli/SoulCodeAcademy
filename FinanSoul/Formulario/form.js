...
class Validator {
  ...
  validateyear(input) {
    ...
    let currentDate = new Date(2021, 2, 16);

    let dataUser = new Date(input.value, this.currentMonth - 1, this.currentDay);

    let age = Math.round((currentDate - dataUser) / 1000 / 60 / 60 / 24 / 365);
...
submit.addEventListener('click', function (e) {
  ...
  if (validator.validate(form)) {
    ...
    você tem ${validator.currentAge} se define como uma pessoa do sexo
    ${validator.currentSelect} e pode usar ${validator.currentCPF} como senha.`);
